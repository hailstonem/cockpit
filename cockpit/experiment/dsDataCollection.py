...
class BiasImageDatasetExperiment(immediateMode.ImmediateModeExperiment):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            self.saveBasePath = os.path.join(os.path.expanduser("~")+'/Data/', saveprefix)
        ...
        self.aodev = depot.getDeviceWithName("ao")  # IS THIS THE CORRECT DEVICE NAME?
    ...
    def executeRep(self, repNum):
        ...
        print(f"Started rep {repNum+1}/{self.numReps} Time Elapsed: {t_elapsed:.1f}s Time Remaining: {(self.numReps-repNum)*t_elapsed/(repNum+1)/60:.1f} min")
        ...
        for area in range(areas):
            ...
            for applied_abb in applied_modes:
                for step in applied_steps:
                    current_aberrations=start_aberrations.copy()
                    current_aberrations[applied_abb] = step
                    biaslist = self.makeBiasPolytope(current_aberrations, bias_modes, len(current_aberrations),steps=(1,2))
                    fprefix = f"R{areas}A{area}A{applied_abb}S{step:.2f}_"
